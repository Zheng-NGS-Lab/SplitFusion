...
if [[ "$mawkver" != "1.3.4" || "$awk" == "" ]]; then
  ...
  if [ "$awk" == "" ]; then
	...
	#==== if no umi, add umi:A for compatability
			if [ $hasUmi -eq 0 ]; then
				$bwa mem $bwaOpts -h 150,200 -q -K 10000000 -t $thread $refGenome $fastq_file1 $fastq_file2 2> bwa.log | $awk '/^@/{print}!/^@/{OFS="\t";if(preId!=$1){n++}; preId=$1; $1=$1":umi:A"n; print $0}' | $awk -F"\t" -v w=$SFpath -v v=$genomeVer -f $SFpath/exec/pseudogene.awk | $samtools view -@ $thread -bS - > _raw.bam
			else
				$bwa mem $bwaOpts -h 150,200 -q -K 10000000 -t $thread $refGenome $fastq_file1 $fastq_file2 2> bwa.log | $awk -F"\t" -v w=$SFpath -v v=$genomeVer -f $SFpath/exec/pseudogene.awk | $samtools view -@ $thread -bS - > _raw.bam
			fi
		else	
			if [ $hasUmi -eq 0 ]; then
				$bwa mem $bwaOpts -h 150,200 -q -K 10000000 -t $thread $refGenome $fastq_file1 2> bwa.log | $awk '/^@/{print}!/^@/{OFS="\t";if(preId!=$1){n++}; preId=$1; $1=$1":umi:A"n; print $0}' | $awk -F"\t" -v w=$SFpath -v v=$genomeVer -f $SFpath/exec/pseudogene.awk | $samtools view -@ $thread -bS - > _raw.bam
			else
				$bwa mem $bwaOpts -h 150,200 -q -K 10000000 -t $thread $refGenome $fastq_file1 2> bwa.log | $awk -F"\t" -v w=$SFpath -v v=$genomeVer -f $SFpath/exec/pseudogene.awk | $samtools view -@ $thread -bS - > _raw.bam
